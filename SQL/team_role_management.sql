...
begin
  ...
  if v_status not in ('active', 'inactive', 'suspended', 'blocked') then
  ...
  if v_actor_role <> 'owner' and (v_target_role = 'owner' or v_role = 'owner') then
    raise exception 'TEAM_OWNER_ONLY';
  end if;
